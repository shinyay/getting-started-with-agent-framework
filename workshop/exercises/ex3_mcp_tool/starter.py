...
# Solution reference: src/demo3_hosted_mcp.py
...
def _require_command(cmd: str) -> str:
    ...
    if not resolved:
        raise RuntimeError(
            ...
            "Exercise 3 uses a local MCP server launched via Node.js (npx).\n"
...
async def main() -> None:
    ...
    # ----------------------------------------------------------------
    # TODO(2): Verify that 'npx' is available on PATH using _require_command()
    # ----------------------------------------------------------------
    ...

    # ----------------------------------------------------------------
    # TODO(3): Create an MCPStdioTool for "sequential-thinking" server
    #   Hint: command="npx", args=["-y", "@modelcontextprotocol/server-sequential-thinking"], load_prompts=False
    # ----------------------------------------------------------------
    ...
